        ...
        setItems((prev) =>
          prev.map((item) => {
            ...
            if (item.thresholdHigh != null && current >= item.thresholdHigh && !item.notifiedHigh) {
              sendNotification("Price Alert", `${item.itemName} is now ${(current / 1000).toFixed(0)}K (above ${(item.thresholdHigh / 1000).toFixed(0)}K)`);
              updated = { ...updated, notifiedHigh: true };
            }
            if (item.thresholdLow != null && current <= item.thresholdLow && !item.notifiedLow) {
              sendNotification("Price Alert", `${item.itemName} is now ${(current / 1000).toFixed(0)}K (below ${(item.thresholdLow / 1000).toFixed(0)}K)`);
