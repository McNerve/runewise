...
function categorizeChangelog(body: string | null): ChangelogSections | null {
  if (!body) return null;
  const lines = body
    .split("\n")
...
function parseChangelogFallback(body: string | null): string[] {
  if (!body) return [];
  return body
    .split("\n")
    .map((l) => l.replace(/^[-*•]\s*/, "").trim())
...
function VersionChevron({ from, to }: { from: string; to: string }) {
  return (
    <div className="flex items-center justify-center gap-2 mt-1.5 mb-0.5">
      ...
      <svg viewBox="0 0 16 10" className="w-4 h-3 text-accent" fill="none">
...
function ChangelogView({ body }: { body: string | null }) {
